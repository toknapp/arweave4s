...
class apiExamples extends WordSpec
  ...
  "An api axample" should {
    "be able to use Id" taggedAs(Slow, Retryable) in {
      ...
      implicit val c = api.Config(hosts = TestHost :: Nil, HttpURLConnectionBackend(), retries = 1)
      import api.id._
    ...
    "be able to use for-comprehensions" taggedAs(Retryable) in {
      ...
      implicit val c = api.Config(hosts = TestHost :: Nil, AsyncHttpClientFutureBackend(), retries = 1)
      import api.future._
